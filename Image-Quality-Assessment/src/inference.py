...
def get_image_quality_scores(img_path):
    import joblib
    import os
    import cv2
    from blur_functions import varMaxLaplacian, varMaxSobel
    ...
    model_path = os.path.join(base_dir, '..', 'model.joblib')  # adjust if it's elsewhere
    ...
    blur_score = float(model.predict_proba([[lapmax, lapvar, sobmax, sobvar]])[0][1]) * 100

    return {
        "label": class_label,
        "score": round(blur_score, 2),
        "laplacian_var": lapvar,
        "laplacian_max": lapmax,
        "sobel_var": sobvar,
        "sobel_max": sobmax
    }
